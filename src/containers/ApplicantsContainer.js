...
const ApplicantsContainer = (props) => {
    ...
    const applicants = job.applicants.map(applicant => <div  key={applicant.id}><Applicant applicant={applicant} userId={props.user.id} jobId={job.id} hire={props.hire} history={props.history}/><br/></div>);
    return (
        <div>
            <h4>Applicants:</h4>
